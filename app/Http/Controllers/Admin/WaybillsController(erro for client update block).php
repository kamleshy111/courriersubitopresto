        ...
        if ($validator_checker) {
            ...
            for($i = 0; $i <= $request->get('counter', 0); $i++) {
                    ...
                    $phone = $request->input($i.'.shipper.phone');
                        ...
                        if ($existingClient) {
                            return back()->withErrors([
                                $i.'.shipper.phone' => 'A client with this phone already exists.',
                            ])
                            ->withInput();
                        }
                ...
                if(is_null($model->recipient_id) && $request->has($i.'.recipient')) {
                    ...
                    $phoneRecipient = $request->input($i.'.recipient.phone');

                        $existingClientRecipient = Client::where('phone', $phoneRecipient)->first();
                        
                        if ($existingClientRecipient) {
                            return back()->withErrors([
                                $i.'.shipper.phone' => 'A client with this phone already exists.',
                            ])
                            ->withInput();
                ...
                // old working saved note parmanent to that client stopped as per request 15.12.25
