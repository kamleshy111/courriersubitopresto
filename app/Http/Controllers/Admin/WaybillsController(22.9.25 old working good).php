    ...
    Log::info('Validation checker:', ['value' => $validator_checker]);
        ...
        if ($validator_checker) {
            ...
            for($i = 0; $i <= $request->get('counter', 0); $i++) {
                ...
                // old not saving clients for rapid waybills
                
                /*if(is_null($model->shipper_id) && $request->has($i.'.shipper')) {
                    // $client = Client::updateOrCreate(
                    $client = Client::create(
                        // old phone number checks for update if mactch else create a new one but create function receives one array only no check 
                ...
                // new for rapid waybill client creation
