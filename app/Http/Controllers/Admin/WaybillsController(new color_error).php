...
//                });
    ...
    /*public function showDriverPdf($id){
    {
    // Fetch the Waybill and related models
    $waybill = Waybill::with('user.client', 'shipper', 'recipient')
        ->findOrFail($id);

    // Get the query parameter 'waybill'
    $type = \request()->query('waybill');

    // Check if the type is "false" or "true"
    if ($type == "false" || $type == "true") {
        // Generate PDF and return the response as inline (in-browser PDF)
        $pdf = $this->pdf($waybill, false, 1, true);

        // Return the PDF inline so that it is displayed in the browser
    ...
    // If the type doesn't match, abort with a 403 error
    abort(403);
}*/
...
public function showDriverPdf($id)
{
    try {
        ...
        if ($type == "false" || $type == "true") {
            ...
            // Check if the view exists before loading it
            if (!view()->exists('waybill.pdf')) {
                Log::error('PDF view does not exist.');
                abort(500, 'PDF view does not exist.');
            }

            $pdf->loadView('waybill.pdf', compact('waybill'));
...
//            $waybillsLastCreated = session()->pull('waybills_last_created', []);
    ...
    public function pdf($waybills, $return = false, $copies = 3, $print = false)
    {
        ...
        // $pdf->SetTitle("Waybill (" . $waybill->soft_id . ")");
    ...
    /*public function generateSignedPdfUrl($waybill)
    {
         $waybill = Waybill::findOrFail($waybillId);

    // Generate the signed URL for the waybill PDF route
    $signedUrl = URL::signedRoute('waybill.pdf.signed.link', [
        'waybill' => $waybill->id,
        'waybill=true' => true // Pass the query parameter from the original URL
    ]);
    ...
    return response()->json([
        ...
        //  return response()->json([
        //     'status' => 'success',
        //     'message' => 'WaybillsController is accessible!',
        // ]);
    }*/
    
    public function generateSignedWaybillUrl($waybillId)
    {
        // Find the waybill by ID
        $waybill = Waybill::findOrFail($waybillId); // Ensure the waybill exists

        // Generate the signed URL for the waybill PDF route, including the query parameter 'waybill=true'
        $signedUrl = URL::signedRoute('waybill.pdf.signed.link', [
            'waybillId' => $waybill->id,
            'waybill' => 'true'  // Add the 'waybill=true' query parameter
        ]);
...
public function uploadPickupImage(Request $request, $id)
{
    ...
    if ($request->hasFile('pickup_image')) {
        ...
        Waybill::where('id', $id)->update(['pickup_image' => $path, 'delivery_status' => 2]);
