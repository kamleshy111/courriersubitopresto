...
{
    try {
        $waybill = Waybill::with('user.client', 'shipper', 'recipient')->findOrFail($id);
        $type = \request()->query('waybill');

        if ($type == "false") {
            $pdf = $this->pdf($waybill, false, 1, false); // No forced printing
        } elseif ($type == "true") {
            $pdf = $this->pdf($waybill, false, 3, false); // No forced printing
        } else {
            abort(403);
        }

        return response($pdf->Output('waybill.pdf', 'I')) // 'I' = Open in browser
    ...
    public function pdf($waybills, $return = false, $copies = 3, $print = false)
{
    ...
    // Ensure PDF is output inline, not forcing print
    if ($return) {
        return $pdf->Output('', 'S'); // Return as a string if needed
    }

    // Output PDF inline in the browser (no print dialog, just view)
    return response($pdf->Output('', 'I'))  // 'I' = Inline view in the browser
        ->header('Content-Type', 'application/pdf')
        ->header('Content-Disposition', 'inline; filename="waybill.pdf"');  // Ensure inline viewing
}
