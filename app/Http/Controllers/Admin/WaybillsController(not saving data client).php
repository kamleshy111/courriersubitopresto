...
class WaybillsController extends CRUDController {
    ...
    public function store(Request $request)
    {
        ...
        $rules = $model->rules;
        // $rules_rapid = $model->rules_rapid;

        
        
        /*if ($request->filled('user_id')) {
            $rules['user_id'] = 'required|numeric';
            // Log::info('user_id added in the rules.');
            // Log::info('Rules: ', $rules);
            
            
            $validator_checker = false;
        } else {
            unset($rules['user_id']); // or make it optional
            // Log::info('Rules: ', $rules);
            // $validator = Validator::make($request->except(['counter']), $rules);
            $validator = Validator::make($request->except(['counter']), (new $this->model())->rules);
            $validator_checker = $validator->fails();
            Log::info('validation checker: ', $validator_checker);
            // $validator_checker = false;
        ...
        if ($request->filled('user_id')) {
    // $rules['user_id'] = 'required|numeric';
    // $validator = Validator::make($request->except(['counter']), (new $this->model())->rules_rapid);
    // $validator = Validator::make($request->except(['counter']), (new $this->model())->rules);
    // $validator_checker = $validator->fails();
        ...
        if ($validator_checker) {
            ...
            for($i = 0; $i <= $request->get('counter', 0); $i++) {
                ...
                if(is_null($model->shipper_id) && $request->has($i.'.shipper')) {
                    ...
                    // $shipperClient = Client::updateOrCreate(
                        [
                    ...
                    // $model->shipper_id = $shipperClient->id;
                    $model->shipper_id = $client->id;
                    Log::infO("new shipper created!");
                }

                if(is_null($model->recipient_id) && $request->has($i.'.recipient')) {
                    // $client = Client::updateOrCreate(
                    $recipientClient = Client::updateOrCreate(
                        [
                    ...
                    $model->recipient_id = $recipientClient->id;
                    Log::infO("new recipient created!");
                }
                ...
                /*if ($request->has('shipper_note')) {
                    Log::info("shipper note found ");
                    $model->shipper_note = $request->input('shipper_note');

                    // Log::info("shipper note found ");
                    if(is_null($model->shipper_id)){
                        // $model->shipper_id = $client->id;
                        Log::info("new shipper within shipper note condition ");
                            // $shipper = Client::find($client->id);
                            if ($shipperClient) {
                                Log::info("new shipper client id found: ", $shipperClient);

                                $shipperClient->note_permanent = $request->input('shipper_note');
                                $shipperClient->save();
                            }
                        ...
                        if ($model->shipper_id) {
                            ...
                            if ($shipper) {
                                $shipper->note_permanent = $request->input('shipper_note');
                                $shipper->save();
                            }
                        }
                    }
                ...
                if ($request->has('shipper_note')) {
                    $model->shipper_note = $request->input('shipper_note');
                    Log::shipper("shipper note found ");
                    // Log::shipper("shipper with ID: " . $model->shipper_id);
                    if(is_null($model->shipper_id)){
                        // $model->shipper_id = $client->id;

                            $shipper = Client::find($client->id);
                            if ($shipper) {
                                Log::shipper("shipper ID: ", $shipper);
                                $shipper->note_permanent = $request->input('shipper_note');
                                $shipper->save();
                            }
                    ...
                    if(is_null($model->recipient_id)){
                        // $model->shipper_id = $client->id;
                            ...
                            if ($recipientClient) {
                                ...
                                $recipientClient->note_permanent = $request->input('shipper_note');
                                $recipientClient->save();
                            }
                        ...
                        if ($model->recipient_id) {
                            ...
                            if ($recipient) {
                                $recipient->note_permanent = $request->input('shipper_note');
                                $recipient->save();
                            }
                        }
                    }
