...
/*public function assignDriver (Request $request) {
    ...
    if ($waybill && !$request->delivery_status) {
        ...
        $waybill->driver_id = $request->driver_id;
        $waybill->driver_assign_time = now();
...
public function checkMiraDriverStatus($id)
{
    $waybill = Waybill::findOrFail($id);
    return response()->json([
        'driver_id' => $waybill->driver_id,
        'is_mira' => $waybill->is_mira, // or 'delivery_status' if that's the column name
...
public function assignDriver (Request $request) {
    ...
    if ($waybill && !$request->delivery_status) {
        ...
        if($waybill->is_mira == 1 && ( $waybill->delivery_status == 1 ) )
            {
                return response()->json(['success' => true, 'message' => 'Mira movement detected.']);
            }
        else{
        // Update the driver_id
        $waybill->driver_id = $request->driver_id;
...
//        $today = Carbon::today(); // very_old
        ...
        $waybills = Waybill::where('approval_status', true)
        ->orderByRaw('ISNULL(`order`), `order` ASC, `id` ASC') // NULLs last
        // ->take(50)
        ->get();
    ...
    public function updateOrder(Request $request)
    {
        ...
        foreach ($orders as $item) {
            Waybill::where('id', $item['id'])->update(['order' => $item['order']]);
        }

        return response()->json(['status' => 'success']);
    }
    
    public function updatePopupOrder(Request $request)
    {
        $orders = $request->input('orders');

        foreach ($orders as $item) {
            Waybill::where('id', $item['id'])->update(['popup_position' => $item['popup_position']]);
        }

        return response()->json(['status' => 'success']);
    }

    public function getPopupPositions(Request $request)
    {


        $positions = DB::table('waybills')
            ->where('approval_status', '1') // Change column name as needed
            ->select('id', 'popup_position')
            ->orderByRaw('ISNULL(popup_position), popup_position ASC') // Sort nulls last
            ->get();
...

{
    ...
    $validated = $request->validate([
        ...
        $allowedColumns = ['user_id', 'status', 'phone', 'contact', 'address', 'city_name', 'postal_code', 'stickyNote_day', 'description' , 'weight_1', 'truck_1', 'waiting_time_1','note_permanent','dashboard_position','cheque_note','notes','dashboard_price','comission_price', 'shipper_note','recipient_note','shipper_contact', 'recipient_contact','address_ext']; // Replace with your columns
        ...
        try {
            ...
            if ($tableName == 'waybills'){
            
                DB::table($tableName)

                ->where('id', $selectedNoteId)

                ->update([$columnName => $newValue, 'popup_position' => null]);    
            }
            else{
