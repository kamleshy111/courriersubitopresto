...
public function emailReport(Request $request){
    ...
    if(!empty($waybillsToSend)) {
        $htmlContent = "Bonjour Courrier Subito presto,<br>Le chauffeur a termin� sa journ�e et a transmis son bordereau de livraison.";
                ...
                Log::info('Reordered waybill IDs driver email:', $waybills->pluck('id')->toArray());
                    ...
                    /*try {Mail::html("", function ($message) use ($pdf) {
                        ...
                        // ->bcc('ali2015333061@gmail.com')
                            ...
                            // $message->to('ali2015333061@gmail.com')
                                // ->bcc('widmaertelisma@gmail.com')
                            ->subject(' Fin de journ�e du chauffeur. Bordereau en pi�ce jointe ' . \Auth::user()->name)
                    ...
                    $message->setBody('T�l�chargez le fichier ici : ' . $pdfUrl, 'text/plain');
                }
                });*/
    ...
    Mail::send([], [], function ($message) use ($pdfContent, $pdfSizeBytes, $maxSizeBytes, $pdfUrl) {
        ...
        $message->to('danybergeron@courriersubitopresto.com')
                ->bcc('ali2015333061@gmail.com')
                ->subject('Fin de journ�e du chauffeur. Bordereau en pi�ce jointe ' . (\Auth::check() ? \Auth::user()->name : 'Utilisateur inconnu'));
        ...
        if ($pdfSizeBytes <= $maxSizeBytes) {
            ...
            // $message->setBody('T�l�chargez le fichier ici : ' . $pdfUrl, 'text/plain');
            $message->setBody('T�l�chargez le fichier ici : <a href="' . $pdfUrl . '">' . $pdfUrl . '</a>', 'text/html');
