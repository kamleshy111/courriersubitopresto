        ...
        return datatables( $result)
            ...
            ->editColumn('action', function ($row) use($request) {
                ...
                if($request->query('waybill-type') && $request->query('waybill-type') == "true"){
                    ...
                    $button .= '<a class="btn btn-sm btn-primary" style="background-color:#FF5733;" target="_blank" href="' . url('admin/waybill/' . $row->id) . '"><i class="fas fa-file-pdf"></i> Bordereau</a>';

                }

                if($request->query('waybill-type') && $request->query('waybill-type') == "true"){

                    $button .= '<a class="btn btn-sm btn-primary"  target="_blank" href="' . url('admin/waybills/' . $row->id . '?waybill=' . \request()->query('waybill-type')) . '"><i class="fas fa-eye"></i> Voir Waybill</a>';
...

{
    ...
    return datatables($waybills)
        ...
        ->editColumn('action', function ($row) {
            ...
            $button .= '<a class="btn btn-sm btn-info" title="voir la feuille de route" style="width: 2rem; padding: 8px 0; margin-right: 3px; margin-left: 3px;" href="' . url('admin/waybill/' . $row->id) . '"><i class="fa fa-eye"></i></a>';
