...
@php
    $clientId = request()->segment(3);  // Alternative syntax for Request::segment()
    // echo $clientId;
    @endphp
{{-- @php $clientId = $client->id ?? $clientId ?? null; @endphp --}}


<div class="card mt-3">
    <div class="card-header">
        <strong>Document PDF du client</strong>
    </div>

    <div class="card-body">

        {{-- Upload form --}}
        {{-- <form action="{{ route('admin.clients.store_pdf', $client) }}" --}}
      {{-- method="POST" enctype="multipart/form-data"> --}}
      {{-- @csrf --}}

      <form action="{{ route('admin.clients.store_pdf', $clientId) }}"
      method="POST" enctype="multipart/form-data">
            @csrf
