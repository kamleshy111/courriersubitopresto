            ...
            @forelse($waybills as $waybill)
                <div class="section-container">
                    <div style="display: flex; align-items: center;">
                        ...
                        <span id="waybill-meta-{{ $waybill->id }}"
      data-waybill-id="{{ $waybill->id }}"
      data-is-mira="{{ $waybill->is_mira?? "" }}"
      ></span>
                        @if($waybill->delivery_status == 1)
                            ...
                            @else
                                ...
                                <form id="receiver_signature" action="{{ route('admin.waybills.uploadSignatureNote', $waybill->id) }}" method="POST" >
                                    ...
                                    @if($waybill->receiver_textSignature)
                                        ...
                                        <textarea id = "receiver_textSignature" name="receiver_textSignature" class="form-control mb-2" placeholder="{{ $waybill->receiver_textSignature }}" required></textarea>
                                    @else
                                        <textarea id = "receiver_textSignature" name="receiver_textSignature" class="form-control mb-2" placeholder="Entrez votre signature ici" required></textarea>
                                    @endif
...
// cam test 04
    ...
    // alert("is_mira: " + typeof(is_mira)); // Will alert "" or "0" or "1
...
function closeCamera() {
    ...
    // savePickupButton.addEventListener('click',  function() {
        ...
        // console.log("here is mira value: " + is_mira);
    ...
    /*savePickupButton.addEventListener('click', async function () {
        ...
        const shipperSignatureInput = document.getElementById('sender_textSignature');
        const shipperSignature = shipperSignatureInput.value.trim();

        const pickupImageBase64 = pickupImagePath.value; // from camera capture
        const pickupFileInput = document.getElementById('pickup_file_input');
        const uploadedFile = pickupFileInput.files[0]; // from file upload

        const span = document.getElementById('waybill-meta-{{ $waybill->id }}');
        const is_mira = span.dataset.isMira;

        // Validation: Check if image is provided (via camera or file)
        ...
        // Validation: Check if signature is valid
        if (!shipperSignatureInput.checkValidity()) {
        ...
        try {
            ...
            if (pickupImageBase64) {
                // Use captured image
                imagePayload = pickupImageBase64;
            } else if (uploadedFile) {
                // Convert uploaded file to Base64
                imagePayload = await toBase64(uploadedFile);
            }
    ...
    const pickupImageBase64 = pickupImagePath.value; // from canvas capture
    const pickupFileInput = document.getElementById('pickup_file_input');
    const uploadedFile = pickupFileInput.files[0]; // from file upload
    ...
    try {
        ...
        if (uploadedFile) {
            ...
            const blob = base64ToBlob(pickupImageBase64);
            const file = new File([blob], 'captured_image.jpg', { type: 'image/jpeg' });
            formData.append('pickup_image', file);
        }

        formData.append('is_mira', is_mira);

        // Upload signature and image in parallel
        const [signatureResponse, imageResponse] = await Promise.all([
            fetch("{{ route('admin.waybills.uploadSignatureNote', $waybill->id) }}", {
                method: 'POST',
                headers: {
                    "Content-Type": "application/json",
...
// works uploading files
    ...
    const pickupImageBase64 = document.getElementById('pickupImagePath')?.value || ""; // camera capture
    ...
    try {
        // === Submit Signature ===
        const signatureResponse = await fetch("{{ route('admin.waybills.uploadSignatureNote', $waybill->id) }}", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        if (uploadedFile) {
            ...
            imageUploadResponse = await fetch("{{ route('admin.waybills.uploadPickupImageUpdated', $waybill->id) }}", {
                ...
                body: JSON.stringify({
                    pickup_image: pickupImageBase64,
                    is_mira: is_mira
                })
            });
        }
...
savePickupButton.addEventListener('click', async function () {
    ...
    try {
        ...
        if (uploadedFile) {
            ...
            // Create FormData to send the image file along with other data
            const formData = new FormData();
            formData.append('pickup_image', imageFile);
            formData.append('is_mira', is_mira);
    ...
    // Utility function to convert uploaded file to base64 old
...
function closeCameraReceiver() {
    ...
    captureButtonReceiver.addEventListener('click', function() {
        ...
        //  stopCamera();
         ...
         canvasReceiver.width = 1280;
         canvasReceiver.height = 720;
...
/*saveDropButton.addEventListener('click', async function () {
        ...
        const receiverSignatureInput = document.getElementById('receiver_textSignature');
        const receiverSignature = receiverSignatureInput.value.trim();
        // console.log(receiverSignatureInput);
        // console.log(receiverSignature);
        // alert(receiverSignatureInput);

        const dropImageBase64 = dropImagePath.value; // from camera capture
        const dropFileInput = document.getElementById('drop_file_input'); // file input field
        const uploadedFile = dropFileInput?.files[0];

        // Validation: check if an image is provided
        ...
        // Validation: check if signature is filled/valid
        if (!receiverSignatureInput.checkValidity()) {
            alert("Please enter a valid receiver signature.");
            document.getElementById('overlay').style.display = 'none';
            return;
        }
...
saveDropButton.addEventListener('click', async function () {
    ...
    try {
        ...
        if (!signatureResponse.ok || !signatureData.success) {
            throw new Error("Receiver signature upload failed.");
        }
        ...
        if (!imageUploadResponse.ok || !imageData.success) {
            throw new Error("Drop image upload failed.");
        }
