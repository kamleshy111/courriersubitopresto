            ...
            <tr>
                ...
                @if(auth()->check() && auth()->user()->roles->contains('id', 3))
                    {{--@if(!empty($waybill->shipper->address))
                        <a href="https://www.google.com/maps/search/?api=1&query={{ urlencode($waybill->shipper->address) }}"
                        target="_blank">
                        {{ $waybill->shipper->address }}
                        </a>--}}
                        
                        @if(!empty($waybill->shipper->address))
                        <a href="https://www.google.com/maps/search/?q={{ urlencode($waybill->shipper->address) }}" target="_blank">
            {{ $waybill->shipper->address }}
        </a>
            @php
                $addressEncoded = urlencode($waybill->shipper->address);
            @endphp
            {{-- <a href="geo:0,0?q={{ $addressEncoded }}"
   onclick="if(!navigator.userAgent.match(/Android|iPhone|iPad|iPod/)) 
             window.open('https://www.google.com/maps/search/?api=1&query={{ $addressEncoded }}', '_blank'); 
             return true;">
   {{ $waybill->shipper->address }}
</a> 
