            ...
            <tr>
                ...
                @if(auth()->check() && auth()->user()->roles->contains('id', 3))
                    @if(!empty($waybill->shipper->address))
                        ...
                        <a href="https://www.google.com/maps/search/?api=1&query={{ $addressEncoded }}" target="_blank">
                    ...
                    {{-- {{ $waybill->recipient->address }} --}}
                     ...
                     @if(auth()->check() && auth()->user()->roles->contains('id', 3))
                        @if(!empty($waybill->recipient->address))
                            @php
                            $addressEncodedRecipient = urlencode($waybill->recipient->address);
                        @endphp
                        
                        <a href="https://www.google.com/maps/search/?api=1&query={{ $addressEncodedRecipient }}" target="_blank">
                            {{ $waybill->recipient->address }}
                        </a>
                        @endif
                    @else
