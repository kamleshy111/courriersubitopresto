        ...
        <table class="table table-bordered table-striped" >
            ...
            <thead>
                  ...
                  @php
                                ...
                                $raw_price = isset($waybill->dashboard_price) ? $waybill->dashboard_price : "0"; // Fallback to "0" if not set
                            ...
                            @php
                                ...
                                // Get the raw price or fallback to "0"
                                $raw_price = $waybill->dashboard_price ?? "0";
                            
                                // Replace comma with dot for European decimal format
                                $normalized_price = str_replace(',', '.', $raw_price);
                            
                                // Remove all non-numeric characters except digits, dot, and minus
                                $cleaned_price = preg_replace('/[^0-9.\-]/', '', $normalized_price);
                            
                                // Convert to float (or 0 if conversion fails)
                                $price = is_numeric($cleaned_price) ? (float) $cleaned_price : 0;
                ...
                <tr>
                    ...
                    <th colspan="1">
                        ...
                        {{ $driver->id === null
                            ? 'N/A'
                            : ($driver->id == 99 ? '01' : ($driver->id == 27 ? '20' : $driver->id))
                        }}
                    </th>
            ...
            <tbody id="waybillTableBody">
                ...
                @foreach($waybills as $waybill)
                    ...
                    <tr data-waybill-id = "{{ $waybill->id }}">
                        ...
                        <td class="editable-cell price-cell" data-table="waybills" data-column="dashboard_price" data-id={{$waybill->id}} data-field="price">
...
function statusConvertion(givenStr) {
    ...
    else if (givenStr.toLowerCase() === 'code rouge') {
        ...
        if(data.isAdmin == 1){
            ...
            if (data.waybills.length === 0) {
                ...
                data.waybills.forEach(waybill => {
                    ...
                    // var raw_price = waybill?.dashboard_price ?? "0"; // Fallback to "0" if null or undefined
                    // var raw_price = (waybill && waybill.dashboard_price != null) ? waybill.dashboard_price : "0";
                    
                    // old format only decimal point(.)
                    ...
                    // Step 1: Replace comma with dot (for "22,50" style inputs)
                    raw_price = raw_price.replace(",", ".");
                    
                    // Step 2: Remove everything except digits, dot, and minus sign
                    var cleanedPrice = raw_price.replace(/[^0-9.-]/g, "");
                    
                    // Step 3: Parse to number (float)
                    var price = Number(cleanedPrice);
                    
                    // Step 4: Fallback to 0 if not a number
                    if (isNaN(price)) {
                        price = 0;
                    }
                    ...
                    row.innerHTML = `
                        ...
                        <td  class="editable-cell price-cell" data-table="waybills" data-column="dashboard_price" data-id=${waybill.id} >${price.toFixed(2)} $</td>
