                    ...
                    <div class="shipper">
                        ...
                        <div class="note-section">
                            ...
                            <div id="cameraSection" style="display:none; text-align:center;">
                                ...
                                <button type="button" id="captureButton" class="btn btn-success mt-2" style="width: 100%; max-width: 480px;">
                                    📸 Capture
                                </button>
...
function closeCamera() {
    ...
    async function openCamera() {
        try {
            ...
            // Try to find a rear camera (avoid ultra-wide if possible)
            const backCamera = videoDevices.find(d => /back|rear/i.test(d.label))
                              || videoDevices.find(d => /environment/i.test(d.label))
            ...
            const constraints = {
                video: {
                    ...
                    // /*width: { ideal: 1920 }, // higher resolution for better stills
                    // height: { ideal: 1080 },
                    width: { ideal: 1280 }, // higher resolution for better stills
                    height: { ideal: 720 },
                    advanced: [{ focusMode: "continuous" }] // iOS Safari tries to honor
                }
            };
    ...
    captureButton.addEventListener('click', function () {
        ...
        capturedImage.src = imageData;
        capturedImage.style.display = 'block';
        pickupImagePath.value = imageData;
        closeCamera();
    });
    ...
    async function openCamera() {
        try {
            // First request to trigger permission popup (labels empty before this)
            await navigator.mediaDevices.getUserMedia({ video: true });
    
            // Now labels should be visible
            const devices = await navigator.mediaDevices.enumerateDevices();
            const videoDevices = devices.filter(device => device.kind === 'videoinput');
    
            // Pick the first back camera that does NOT contain "wide" or "ultra" in label
            let backCamera = videoDevices.find(d =>
                /back|rear|environment/i.test(d.label) &&
                !/wide|ultra/i.test(d.label)
            );
    
            // Fallback: just pick the first back/environment camera
            if (!backCamera) {
                backCamera = videoDevices.find(d => /back|rear|environment/i.test(d.label))
                           || videoDevices[0];
            }
...
async function openCamera() {
  try {
    // Request permission and get back cameras
    await navigator.mediaDevices.getUserMedia({ video: true });
    const devices = await navigator.mediaDevices.enumerateDevices();
    const videoDevices = devices.filter(d => d.kind === 'videoinput');
    let backCamera = videoDevices.find(d => /back|rear|environment/i.test(d.label));
    ...
    // Prepare ImageCapture for high-res photo
...
captureButton.addEventListener('click', async () => {
  if (imageCapture) {
    try {
      ...
      reader.onloadend = () => {
        pickupImagePath.value = reader.result;
      };
      ...
      console.warn('ImageCapture failed, falling back to canvas capture:', error);
    }
  }

  // Fallback to canvas capture if ImageCapture not supported or failed
