 @extends('adminlte::page')

@section('content_header')
    

    
    

@endsection


@section('content')

<div class="container">
    
    
               
    <!-- Wrapper to hold both tables -->
    <div class="waybill-container">
        <!-- Table 1: Shipper & Recipient Info -->
        <div class="waybill-table">
            
            <table id="left-table">
            <tr>
                <td align="center">
                    <img src="data:image/png;base64," />
                </td>
            </tr>
            <tr>
                <td align="center">
                    (NIR): R5608402
                </td>
            </tr>
            <tr>
                <td style="border: 1mm solid black; padding: 10px; ">
                    <p><u>EXPÉDITEUR</u></p>
                    <table width="100%">
                        <tr>
                            <td><p>{{ $shipper->name ?? 'N/A' }}</p></td>
                            {{-- <td><p>{{preg_replace('~.*(\d{3})[^\d]{0,7}(\d{3})[^\d]{0,7}(\d{4}).*~', '($1) $2-$3', $shipper['phone']).(is_null($shipper['extension']) ? '' : 'x'.$shipper['extension'])}}</p></td> --}}
                            <td><p>test value</p></td>
                        </tr>
                        <tr>
                            <td colspan="2"><p>{{ $shipper->address ?? 'N/A' }}</p></td>
                        </tr>
                        <tr>
                            <td colspan="2">
                                <table width="100%">
                                    <tr>
                                        <td align="left"><p>{{$shipper->city_name ?? 'N/A' }}</p></td>
                                        <td align="center"><p>{{$shipper->city_state ?? 'N/A' }}</p></td>
                                        <td align="right"><p>{{$shipper->postal_code ?? 'N/A' }}</p></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2"><p>{{$shipper->contact??'Aucune information de contact supplémentaire'}}</p></td>
                        </tr>
                        {{--<tr>
                            <td colspan="2"><p>&nbsp;</p><p>&nbsp;</p></td>
                        </tr>--}}
                        <tr>
                            <td colspan="2"><p>Signature: ________________________ Heure: ____________</p></td>
                        </tr>
                    </table>
                </td>
            </tr>
            <tr>
                <td>&nbsp;</td>
            </tr>
            <tr>
                <td style="border: 1mm solid black; padding: 10px">
                    <p><u>DESTINATAIRE</u></p>
                    <table width="100%">
                        <tr>
                            <td><p>{{$recipient->name?? 'N/A'}}</p></td>
                            {{-- <td><p>{{preg_replace('~.*(\d{3})[^\d]{0,7}(\d{3})[^\d]{0,7}(\d{4}).*~', '($1) $2-$3', $recipient['phone']).(is_null($recipient['extension']) ? '' : 'x'.$recipient['extension'])}}</p></td> --}}
                            <td><p>test rec</p></td>
                        </tr>
                        <tr>
                            <td colspan="2"><p>{{$recipient->address?? 'address'}}</p></td>
                        </tr>
                        <tr>
                            <td colspan="2">
                                <table width="100%">
                                    <tr>
                                        <td align="left"><p>{{$recipient->city_name??'city_name not found'}}</p></td>
                                        <td align="center"><p>{{$recipient->city_state??'n/a'}}</p></td>
                                        <td align="right"><p>{{$recipient->postal_code??'n/a'}}</p></td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2"><p>{{$recipient->contact??'Aucune information de contact supplémentaire'}}</p></td>
                        </tr>
                        {{--<tr>
                            <td colspan="2"><p>&nbsp;</p><p>&nbsp;</p></td>
                        </tr>--}}
                        <tr>
                            <td colspan="2"><p>Signature: ________________________ Heure: ____________</p></p></td>
                        </tr>
                    </table>
                </td>
            </tr>
            <tr>
                <td style="padding-top:20px;">
                    <p><b>Note permanente</b></p>
                    <p style="background: {{ isset($waybill->note_permanent) ? 'yellow' : 'white'}};font-size:6mm;">{{$waybill->note_permanent??'N/D'}}</p>
                </td>
            </tr>
        </table>
        </div>

        <!-- Table 2: Cost & Payment Details -->
        <div class="waybill-table">
            <table id="right-table">
                        <tr>
                            <td style="border: 1mm solid black; padding-right: 10px;">
                                <table width="100%" >
                                    <tr>
                                        <td>
                                            <p><u>DATE</u></p>
                                            <p>{{ Carbon\Carbon::parse($waybill->date)->format('Y-m-d') }}</p>

                                            <p>No. Client: </p>
                                        </td>
                                        <td valign="top">
                                            <p><u>PAYÉ PAR:</u> <b>{{__('waybills.who_pay.'.$waybill->who_pay)}}</b></p>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td colspan="3">
                                            <p>{{$waybill->description??'Aucune description'}}</p>
                                        </td>
                                    </tr>
                                </table>
                            </td>
                        </tr>
                        <tr>
                            <td style="padding-top: 30px">
                                <table width="100%" cellpadding="5" cellspacing="5" style="border: 1mm solid black; padding: 10px;">
                                    <tr>
                                        <td style="border: 0.5mm solid black;" colspan="3" align="right">Coût</td>
                                    </tr>
                                    <tr>
                                            <td style="border: 0.5mm solid black;" width="50%">Tarif</td>
                                              @php
                                                $formattedPrice = str_replace(',', '.', $waybill->price);
                                               @endphp
                                            <td style="border: 0.5mm solid black;" width="25%">
                                                @role('admin'||'Driver')
                                                     {{ number_format(floatval($formattedPrice), 2) }} $
                                                @else
                                                    @if($waybill->submission_status == 0 && $waybill->price != null)
                                                        {{ number_format(floatval($formattedPrice), 2) }} $
                                                    @endif
                                                @endrole

                                            </td>
                                            @role('admin'||'Driver')
                                                <td style="border: 0.5mm solid black;" width="25%">{{$waybill->cost_2}}</td>
                                            @else
                                                @if($waybill->submission_status == 0)
                                                    <td style="border: 0.5mm solid black;" width="25%"></td>
                                                @endif
                                            @endrole

                                        </tr>
                                    <tr>
                                        <td style="border: 0.5mm solid black;">Matière dangeureuses</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->hazardous_materials_1}}</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->hazardous_materials_2}}</td>
                                    </tr>
                                    <tr>
                                        <td style="border: 0.5mm solid black;">Poids (lbs)</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->weight_1}}</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->weight_2}}</td>
                                    </tr>
                                    <tr>
                                        <td style="border: 0.5mm solid black;">Cubage</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->cubing_1}}</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->cubing_2}}</td>
                                    </tr>
                                    <tr>
                                        <td style="border: 0.5mm solid black;">Aller/retour</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->waiting_time_1}}</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->waiting_time_2}}</td>
                                    </tr>
                                    <tr>
                                        <td style="border: 0.5mm solid black;">Temps d’attente</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->round_trip_1}}</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->round_trip_2}}</td>
                                    </tr>
                                    <tr>
                                        <td style="border: 0.5mm solid black;">Camion</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->truck_1}}</td>
                                                <td style="border: 0.5mm solid black;">{{$waybill->truck_2}}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="2" align="right">Total</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->total}}</td>
                                    </tr>
                                    <tr>
                                        <td colspan="2" align="right">No.</td>
                                        <td style="border: 0.5mm solid black;">{{$waybill->user->client->prefix.str_pad($waybill->soft_id, 6, 0, STR_PAD_LEFT)}}</td>
                                    </tr>

                                </table>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <p style="font-size: 9mm;">Type d’envoi : <b style="background:yellow"><u>{{$waybill->status}}</u></b></p>
                            </td>
                        </tr>
                        <tr>
                            <td style="padding-top: 20px">
                                <p>Chauffeur: ________________________________ </p>
                            </td>
                        </tr>
                        <tr>
                            <td class="mt-1">
                                <p>Chauffeur: ________________________________  Date: ______________</p>
                            </td>
                        </tr>

                    </table>
        </div>
    </div>
</div>

<style>
.body {
            scroll-behavior: smooth;
            /*color:#000000;*/
        }
.container, .container-fluid, .container-sm, .container-md, .container-lg, .container-xl{
    margin-bottom:30px;
    padding-left:0px;
}
#left-table{
    width: 100%;
    color: #000000;
}

#right-table{
    width: 100%;
    color: #000000;
}
    /* Container to hold both tables side by side */
    .waybill-container {
        display: flex;
        flex-direction: row;
        justify-content: space-between;
        /*align-items: flex-start;*/
         align-items: stretch;
        width: 100%;
        max-width: 1200px; /* Prevents excessive stretching */
        margin: 0 auto; /* Centers the content */
        gap: 10px;
    }

    /* Style for both tables */
    .waybill-table {
        background-color: {{ 
            $status == 'tomorrow' ? '#8fdefb' : 
            ($status == 'same_day' ? '#a6f4ee' : 
            ($status == 'urgent' ? '#FDFD96' : 
            ($status == 'code_red' ? '#e3342f' : '#ffffff'))) 
        }};
       opacity: 90%;
        flex: 1;
        flex-direction: column;
        justify-content: space-between; /* Ensures content is evenly spaced */
        min-width: 45%;
        padding: 10px;
        border: 1px solid #ddd;
        border-radius: 10px;
        /*background: #f9f9f9;*/
        box-shadow: 2px 2px 10px rgba(0, 0, 0, 0.1);
    }

    /* Mobile adjustments */
    @media (max-width: 768px) {
        .waybill-container {
            transform: scale(.35); /* Scales down both tables */
            transform-origin: top left; /* Keeps position stable */
            display: flex;
            flex-wrap: nowrap; /* Ensures they stay side by side */
            overflow-x: auto; /* Allows scrolling if needed */
            width: 1200px; /* Ensures the full layout is maintained */
        }

        .waybill-table {
            min-width: 50%; /* Ensures two equal columns */
            padding: 0px 1.5rem;
        }
    }
</style>

<script>
    // JavaScript to adjust scaling dynamically
    
</script>
@endsection
