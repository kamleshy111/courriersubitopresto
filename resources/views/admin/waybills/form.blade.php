                        ...
                        <fieldset id="{{ $i }}_shipper" @if($errors->has('shipper_id')) style="border-color:red; color:red;" @endif>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm">
                                    ...
                                    {!! Form::text($i.'.shipper.address_ext')->placeholder('Suite ou bureau')->readonly($readonly_shipper) !!}
                        ...
                        <fieldset id="{{$i}}_recipient" class="my-3" {!! $errors->has('recipient_id') ? 'style="border-color:red;color:red"' : '' !!}>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm">
                                    ...
                                    {!! Form::text($i.'.recipient.address_ext')->placeholder('Suite ou bureau')->readonly($readonly_recipient) !!}
...
/*document.addEventListener('DOMContentLoaded', () => {
  const roundTrip = document.querySelector('#inp-0-round_trip_1');
  const addButton = document.querySelector('#add_waybills');
  const removeButton = document.querySelector('#remove_waybills');
  const counterInput = document.querySelector('#inp-counter');

  if (!roundTrip || !addButton || !removeButton) return;

  // Detect change on the round trip checkbox
  roundTrip.addEventListener('change', function() {
    if (this.checked) {
      ...
      setTimeout(() => {
        const fromIndex = 0;
        const toIndex = parseInt(counterInput.value);
        fillReverseWaybill(fromIndex, toIndex);
      }, 700); // Adjust delay if needed
    } else {
      // Remove reverse waybill when unchecked
      removeButton.click();
    }
  });

  /*function fillReverseWaybill(fromIndex, toIndex) {
    const fromInputs = document.querySelectorAll(`[name^="${fromIndex}["]`);
    if (!fromInputs.length) return;

    fromInputs.forEach(input => {
      const fromName = input.name;
      let toName = fromName.replace(`${fromIndex}`, `${toIndex}`);

      // Swap shipper/recipient
      if (fromName.includes('[shipper][')) toName = toName.replace('[shipper][', '[recipient][');
      else if (fromName.includes('[recipient][')) toName = toName.replace('[recipient][', '[shipper][');

      const targetElements = document.querySelectorAll(`[name="${toName}"]`);
      if (!targetElements.length) return;

      if (input.type === 'radio') {
        if (input.checked) {
          targetElements.forEach(radio => {
        ...
        targetElements[0].checked = input.checked;
        targetElements[0].dispatchEvent(new Event('change', { bubbles: true }));
      } else {
        targetElements[0].value = input.value;
        targetElements[0].dispatchEvent(new Event('change', { bubbles: true }));
      }
    });

    // Tick round-trip checkbox on new form
    const newRoundTrip = document.querySelector(`#inp-${toIndex}-round_trip_1`);
    if (newRoundTrip) newRoundTrip.checked = true;

    console.log(`✅ Reverse waybill autofilled (Form ${toIndex})`);
  }
...
// old working
  ...
  fromInputs.forEach(input => {
    ...
    if (input.type === 'radio') {
      ...
      // Copy text fields, but clear IDs
      if (toName.includes('[shipper_id]') || toName.includes('[recipient_id]')) {
        // Reset IDs to force backend to detect/create correctly
        targetElements[0].value = '';
      } else {
        targetElements[0].value = input.value;
      }
...
/*document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!roundTrip || !addButton) return;

  // Add reverse waybill when checked
  roundTrip.addEventListener('change', function() {
    if (this.checked) {
      // Add new form
      addButton.click();
      ...
      setTimeout(() => {
        const fromIndex = 0;
        const toIndex = parseInt(counterInput.value);
        fillReverseWaybillData(fromIndex, toIndex);
      }, 500); // adjust if needed
    } else {
      // Remove the last form when unchecked
      removeButton.click();
    }
  });

  function fillReverseWaybillData(fromIndex, toIndex) {
    const fromInputs = document.querySelectorAll(`[name^="${fromIndex}["]`);
    if (!fromInputs.length) {
      console.warn('No fields found to copy from form index', fromIndex);
      return;
    }
    ...
    fromInputs.forEach(input => {
      ...
      if (input.type === 'checkbox' || input.type === 'radio') {
        fromValue = input.checked;
      } else {
        fromValue = input.value;
      }

      // Swap index
      let toName = input.name.replace(`${fromIndex}`, `${toIndex}`);

      // Swap shipper and recipient fields
      if (toName.includes('[shipper][')) toName = toName.replace('[shipper][', '[recipient][');
      if (toName.includes('[recipient][')) toName = toName.replace('[recipient][', '[shipper][');

      // Swap IDs
      if (toName.endsWith('shipper_id]')) toName = toName.replace('shipper_id]', 'recipient_id]');
      if (toName.endsWith('recipient_id]')) toName = toName.replace('recipient_id]', 'shipper_id]');

      const target = document.querySelector(`[name="${toName}"]`);
      if (!target) return;

      // Fill value
      if (target.type === 'checkbox' || target.type === 'radio') {
        target.checked = fromValue;
      } else {
