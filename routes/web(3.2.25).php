...
/*Route::get('/waybill/{waybillNumber}', function ($waybillNumber) {
    // Fetch the waybill data from the database
    // $waybill = Waybill::where('id', $waybillNumber)->first();
    $waybill = Waybill::with(['shipper', 'recipient'])->where('id', $waybillNumber)->first();
    // Log::info('Waybill Data:', (array) $waybill);
    Log::info('Waybill Data:', $waybill->attributesToArray());
    ...
    // Return the view and pass the waybill data
    // return view('admin/waybills/waybill', compact('waybill'));
    return view('waybill', compact('waybill','shipper', 'recipient', 'status'));
...
// Route::get('/waybill/pdf/{id}', [\App\Http\Controllers\Admin\WaybillsController::class, 'pdf'])
//     ->middleware('auth')
//     ->name('waybill.pdf');
...
/*Route::get('/pdf/view', function() {
    // Define the file path (you can change this to your actual file path)
    $filePath = 'public/waybills_pdf/25216.pdf';

    // Check if the file exists in the storage
    if (Storage::exists($filePath)) {
        // Return the file as a response to the browser to display the PDF
